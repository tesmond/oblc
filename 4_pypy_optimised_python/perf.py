...
def process_line(line, result):
    ...
    if stats is None:
        ...
        stats[0] += 1
        stats[1] += temp_float
        stats[2] = min(stats[2], temp_float)
        stats[3] = max(stats[3], temp_float)
    ...
    for result in results:
        for city, item in result.items():
            try:
                ...
                final[city] = item.copy()
            else:
                city_result[0] += item[0]
                city_result[1] += item[1]

                if item[2] < city_result[2]:
                    city_result[2] = item[2]

                if item[3] > city_result[3]:
                    city_result[3] = item[3]
